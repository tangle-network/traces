...
A corpus is a gold label file plus a prepared directory (`normalized/<traj_id>/steps.json` + `extracted/<traj_id>/swe_raw/**`; raw trajectories may be mini-SWE `*.traj.json` or SWE-agent `*.traj`). Replayable = the raw trajectory carries `info.docker_config.base_image` — or `--image-map PATH` supplies a `trajId → {image, cwd?, imageSource?}` entry for families that record no docker config (SWE-agent) — AND the labels mark ≥1 gold incorrect step that is a real mid-trajectory action. The recorded docker_config always wins over the map, and every case row carries its `imageSource` so externally resolved identities stay auditable. SWE-agent's bare `submit` action counts as a submit decision exactly like mini-SWE's sentinel. The batch:
