...
describe('arm scores', () => {
  ...
  it('never turns an unreported cost into a zero', () => {
    const scored = answers([
      { question: 'op.status-polls', answer: null, cost_usd: 2, cost_basis: 'estimated' },
      { question: 'op.runs', answer: null },
    ])
    expect(scored.canonical.cost).toMatchObject({ reported: 1, missing: 1, total: 2, basis: 'mixed' })
    expect(scored.canonical.falseNotInTrace).toBeGreaterThan(0)
  })

  it('renders one table row per attempted wording', () => {
