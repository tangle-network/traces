...
const ERROR_PROSE = /^(?:error|exception|status)(?:\.(?:message|stacktrace|stack|description|details|reason))?$|^(?:log|event)\.message$|^otel\.status_description$/
...
export function stripContent(spans: readonly OtlpSpan[]): { spans: OtlpSpan[]; dropped: string[] } {
  ...
  const out = spans.map((span) => {
    ...
    for (const [key, value] of Object.entries(span.attributes ?? {})) {
      if (isContentAttribute(key) || TOOL_IO_DERIVED.test(key) || ERROR_PROSE.test(key) || key === 'tool.args_captured') {
        dropped.add(key)
        continue
      }
